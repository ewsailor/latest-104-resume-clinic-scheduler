...
def main():
    ...
    origins_result = check_cors_origins(
        ["http://localhost:3000", "http://localhost:8000", "https://www.104.com.tw"]
    )

    # 檢查方法
    methods_result = check_cors_methods(["GET", "POST", "PUT", "DELETE", "OPTIONS"])
    ...
    # 輸出問題
    all_issues = (
        origins_result['issues'] + methods_result['issues'] + headers_result['issues']
    )
