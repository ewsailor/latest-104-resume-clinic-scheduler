...
class CORSValidator:
    ...
    def validate_origin_string(self, origin_string: str) -> Dict[str, Any]:
        ...
        # 解析來源
        origins = [
            origin.strip() for origin in origin_string.split(",") if origin.strip()
        ]
        ...
        return {
            ...
            "recommendations": self._generate_origin_recommendations(
                valid_origins, invalid_origins
            ),
        }
    ...
    def _generate_origin_recommendations(
        self, valid_origins: List[str], invalid_origins: List[str]
    ) -> List[str]:
        """
        生成來源改進建議。
        ...
        # 檢查本地開發來源
        local_origins = [
            o for o in valid_origins if "localhost" in o or "127.0.0.1" in o
        ]
