...
class TestSettings:
    ...
    def test_104_api_config(self):
        ...
        if settings.has_104_api_config:
            ...
            # 檢查是否缺少必要配置
            assert (
                settings.api_104_base_url is None
                or settings.api_104_client_id is None
                or settings.api_104_client_secret is None
            )
...
class TestGetProjectVersion:
    ...
    def test_get_project_version_success(self):
        """測試成功讀取版本號"""
        mock_pyproject_data = {"tool": {"poetry": {"version": "1.2.3"}}}
