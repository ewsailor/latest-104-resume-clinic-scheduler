...
class Settings(BaseSettings):
    ...
    # ===== 應用程式基本配置 =====
    app_name: str = Field(
        default="104 Resume Clinic Scheduler",
        description="應用程式名稱"
    )
    ...
    docs_url: str = Field(
        default="/docs",
        description="API 文件 URL"
    )
    redoc_url: str = Field(
        default="/redoc",
        description="ReDoc 文件 URL"
    )
    
    # ===== 靜態檔案配置 =====
    static_url: str = Field(
        default="/static",
        description="靜態檔案 URL 路徑"
    )
    static_name: str = Field(
        default="static",
        description="靜態檔案掛載名稱"
    )
    ...
    mysql_host: str = Field(
        default="localhost",
        description="MySQL 主機地址"
    )
    mysql_port: int = Field(
        default=3306,
        description="MySQL 連接埠"
    )
    ...
    mysql_database: str = Field(
        default="scheduler_db",
        description="MySQL 資料庫名稱"
    )
    ...
    aws_s3_bucket: Optional[str] = Field(
        default=None,
        description="AWS S3 儲存桶名稱"
    )
    
    # ===== 104 API 配置 =====
    api_104_base_url: Optional[str] = Field(
        default=None,
        description="104 API 基礎 URL"
    )
    api_104_client_id: Optional[str] = Field(
        default=None,
        description="104 API 客戶端 ID"
    )
    ...
    api_timeout: int = Field(
        default=10,
        description="API 請求總超時時間（秒）"
    )
    api_connect_timeout: int = Field(
        default=5,
        description="API 連接超時時間（秒）"
    )
    api_read_timeout: int = Field(
        default=10,
        description="API 讀取超時時間（秒）"
    )
    
    # ===== 日誌配置 =====
    log_level: str = Field(
        default="INFO",
        description="日誌等級 (DEBUG, INFO, WARNING, ERROR, CRITICAL)"
    )
    log_file: str = Field(
    ...
    smtp_host: Optional[str] = Field(
        default=None,
        description="SMTP 主機地址"
    )
    smtp_port: int = Field(
        default=587,
        description="SMTP 連接埠"
    )
    ...
    def has_104_api_config(self) -> bool:
        """檢查是否有完整的 104 API 配置"""
        return all([self.api_104_base_url, self.api_104_client_id, self.api_104_client_secret])
    ...
    def get_aws_secret_key(self) -> str:
        """安全地取得 AWS 秘密金鑰"""
        return self.aws_secret_access_key.get_secret_value() if self.aws_secret_access_key else ""
    
    def get_104_api_secret(self) -> str:
        """安全地取得 104 API 密鑰"""
        return self.api_104_client_secret.get_secret_value() if self.api_104_client_secret else ""
